...
def evaluate(brain,env,config,game_id):
    ...
    convert_state(state,dic[game_id],dic_global)
    ...
    for estep in range(config.NUM_EVAL_STEPS):
        ...
        #observe
        convert_state(nextstate,dic[game_id],dic_global)
...
def playgame(config):
    env_ = [Environment(1),Environment(2),Environment(3)]
    actions = env_[0].action_size()  #here all 3 game #actions and #objects
    objects = env_[0].object_size()  #here all 3 game #actions and #objects
    config.setnumactions(actions)
    config.setnumobjects(objects)
    config.setvocabsize(env_[0].vocab_size())

    brain = MDQN(config)

    pbar = tqdm(total = config.MAX_FRAMES, desc='Training Progress')
    
    episode_length_ = [0,0,0]
    num_episodes_ = [0,0,0]
    total_reward_ = [0,0,0]
    ...
    while True:
        for env,episode_length,num_episodes,total_reward,ind in zip(env_,episode_length_,num_episodes_,total_reward_,[0,1,2]):
            if env.START_NEW_GAME:
                ...
                convert_state(state,dic[ind],dic_global)
                brain.history[ind].add(state)

            action_indicator = np.zeros(actions)
            object_indicator = np.zeros(objects)
            #predict
            action_index,object_index = brain.getAction(availableObjects,ind,False)
            action_indicator[action_index] = 1
            object_indicator[object_index] = 1
            #act
            ...
            episode_length += 1
            #observe
            convert_state(nextstate,dic[ind],dic_global)
            brain.setPerception(state, reward, action_indicator, object_indicator, nextstate, terminal, ind, False)
            state = nextstate

            if ((terminal)):
                num_episodes += 1
                with open("train_reward_"+str(ind)+".txt", "a") as fp:
                    print >> fp, (total_reward / (num_episodes * 1.0))    
                env.START_NEW_GAME = True
    #####################################################################
            #for evaluating qvalues
            if (brain.timeStep[ind] % config.EVAL == 0) and (brain.timeStep[ind] != 0):
                if (brain.timeStep[ind] / config.EVAL == 1):
                    if not ((os.path.exists("checkStates"+str(ind)+".txt")) and (os.path.getsize("checkStates"+str(ind)+".txt") > 0)):                    
                        assert config.SAMPLE_STATES % config.BATCH_SIZE == 0 
                        assert config.SAMPLE_STATES < brain.memory[ind].count
                        checkStates, _1, _2, _3, _4, _5 = brain.memory[ind].sample()
                        with open("checkStates"+str(ind)+".txt", "w") as fp:
                            cpickle.dump(checkStates,fp)
                    else:
                        with open("checkStates"+str(ind)+".txt", 'r') as fp:
                            checkStates = cpickle.load(fp)

                print checkStates,ind
                evalQValues_a = brain.action_valueT.eval(feed_dict={brain.stateInputT:checkStates,brain.controller_id : ind},session = brain.session)
                maxEvalQValues_a = np.max(evalQValues_a, axis = 1)
                avgEvalQValues_a = np.mean(maxEvalQValues_a)

                with open("evalQValue_a"+str(ind)+".txt", "a") as fp:
                    print >>fp,avgEvalQValues_a

                evalQValues_o = brain.object_valueT.eval(feed_dict={brain.stateInputT:checkStates,brain.controller_id : ind},session = brain.session)
                maxEvalQValues_o = np.max(evalQValues_o, axis = 1)
                avgEvalQValues_o = np.mean(maxEvalQValues_o)

                with open("evalQValue_o"+str(ind)+".txt", "a") as fp:
    ...
    #####################################################################
                ...
                env_eval = env
                if config.TUTORIAL_WORLD:
                    total_reward, nrewards, nepisodes, quest1_reward_cnt, quest2_reward_cnt, quest3_reward_cnt = evaluate(brain, env_eval, config, ind)
                else:
                    total_reward, nrewards, nepisodes, quest1_reward_cnt = evaluate(brain, env_eval, config,ind)
                
                with open("test_reward"+str(ind)+".txt", "a") as fp:
                ...
                if len(env_eval.reward_history)==0 or total_reward > max(env_eval.reward_history):
                    ...
                    brain.saver.save(brain.session, os.getcwd()+'/Savednetworks/'+'network' + '-dqn', global_step = brain.timeStep[2])                
    ...
    #####################################################################
                values = [avgEvalQValues_a,avgEvalQValues_o,total_reward,nrewards,nepisodes,quest1_reward_cnt]
                keys_ = ['average.q_a','average.q_o','average_reward','average_numrewards','number_of_episodes','quest1_average_reward_cnt']
                keys = [key + str(ind) for key in keys_]
