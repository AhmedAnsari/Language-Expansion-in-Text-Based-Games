...
class student:
    def __init__(self, config):
        ...
        self.action_value_01 = tf.nn.rnn_cell._linear(linear_output, self.config.num_actions, 1.0, 0.01, scope="actionN01")
        self.object_value_01 = tf.nn.rnn_cell._linear(linear_output, self.config.num_objects, 1.0, 0.01, scope="objectN01")

        self.action_value_02 = tf.nn.rnn_cell._linear(linear_output, self.config.num_actions, 1.0, 0.01, scope="actionN02")
        self.object_value_02 = tf.nn.rnn_cell._linear(linear_output, self.config.num_objects, 1.0, 0.01, scope="objectN02")

        self.action_value_03 = tf.nn.rnn_cell._linear(linear_output, self.config.num_actions, 1.0, 0.01, scope="actionN03")
        self.object_value_03 = tf.nn.rnn_cell._linear(linear_output, self.config.num_objects, 1.0, 0.01, scope="objectN03")

        self.action_value_11 = tf.nn.relu(self.action_value_01)
        self.object_value_11 = tf.nn.relu(self.object_value_01)

        self.action_value_12 = tf.nn.relu(self.action_value_02)
        self.object_value_12 = tf.nn.relu(self.object_value_02)

        self.action_value_13 = tf.nn.relu(self.action_value_03)
        self.object_value_13 = tf.nn.relu(self.object_value_03)

        self.action_value_1 = tf.nn.rnn_cell._linear(self.action_value_11, self.config.num_actions, 1.0, 0.01, scope="actionN1")
        self.object_value_1 = tf.nn.rnn_cell._linear(self.object_value_11, self.config.num_objects, 1.0, 0.01, scope="objectN1")

        self.action_value_2 = tf.nn.rnn_cell._linear(self.action_value_12, self.config.num_actions, 1.0, 0.01, scope="actionN2")
        self.object_value_2 = tf.nn.rnn_cell._linear(self.object_value_12, self.config.num_objects, 1.0, 0.01, scope="objectN2")

        self.action_value_3 = tf.nn.rnn_cell._linear(self.action_value_13, self.config.num_actions, 1.0, 0.01, scope="actionN3")
        self.object_value_3 = tf.nn.rnn_cell._linear(self.object_value_13, self.config.num_objects, 1.0, 0.01, scope="objectN3")
