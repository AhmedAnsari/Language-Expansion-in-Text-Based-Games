...
class student:
    def __init__(self, config):
        ...
        linear_output = self.mean_pool

        linear_output_21 = tf.nn.relu(tf.nn.rnn_cell._linear(linear_output, int(self.output_embed.get_shape()[2]/4), 1.0, 0.01, scope="linearN21"))
        linear_output_22 = tf.nn.relu(tf.nn.rnn_cell._linear(linear_output, int(self.output_embed.get_shape()[2]/4), 1.0, 0.01, scope="linearN22"))
        linear_output_23 = tf.nn.relu(tf.nn.rnn_cell._linear(linear_output, int(self.output_embed.get_shape()[2]/4), 1.0, 0.01, scope="linearN23"))

        #we calculate the Q values. For the Student Network
        self.action_value_1 = tf.nn.rnn_cell._linear(linear_output_21, self.config.num_actions, 1.0, 0.01, scope="actionN1")
        self.object_value_1 = tf.nn.rnn_cell._linear(linear_output_21, self.config.num_objects, 1.0, 0.01, scope="objectN1")

        self.action_value_2 = tf.nn.rnn_cell._linear(linear_output_22, self.config.num_actions, 1.0, 0.01, scope="actionN2")
        self.object_value_2 = tf.nn.rnn_cell._linear(linear_output_22, self.config.num_objects, 1.0, 0.01, scope="objectN2")

        self.action_value_3 = tf.nn.rnn_cell._linear(linear_output_23, self.config.num_actions, 1.0, 0.01, scope="actionN3")
        self.object_value_3 = tf.nn.rnn_cell._linear(linear_output_23, self.config.num_objects, 1.0, 0.01, scope="objectN3")
